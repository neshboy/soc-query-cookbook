...
const REPO_ROOT = "C:/Users/User/projects/soc-query-cookbook";
...
// This book's initial release ships 28 drafted/reviewed parts only. BOOK-INDEX.md's own
// Appendix Table already lists 5 planned appendix bundles (Master Pattern Index, Behavior x
// Language Coverage Matrix, MITRE ATT&CK Cross-Reference, Pattern/Front-Matter/Hunt-Chain
// Templates, Query-Language Syntax Pointer), but appendices/ is empty (no appendix has been
// authored yet) - that's a future edition, not a build defect. Detect that up front so the
// Appendix Table section is skipped entirely instead of emitting "MISSING FILE" placeholders,
// same pattern used in microsoft-sentinel-operations-handbook's build.
function appendicesExist() {
  const dir = path.join(REPO_ROOT, "appendices");
...
// "### Section X — Name" subheadings - same shape as the other NESHBOY SOC Professional
// Library books. Parse into the same ordered entry stream build_book expects.
function parseIndex() {
  const lines = readFile(INDEX_PATH).split("\n");
  const entries = [];
  const includeAppendices = appendicesExist();
  let skippingAppendixSection = false;
  ...
  for (const raw of lines) {
    ...
    if (line === "## Part Table") {
      ...
      skippingAppendixSection = false;
      continue;
    }
    if (line === "## Appendix Table") {
      if (!includeAppendices) {
        skippingAppendixSection = true;
        continue;
      }
      entries.push({ type: "part", text: "Appendices" });
      skippingAppendixSection = false;
      continue;
    }
    if (skippingAppendixSection) continue;
    ...
    if (line.startsWith("|")) {
      ...
      // split, then restore, so an escaped pipe never shifts later cells in the row.
    ...
    // everything else (prose, "Total: 28 parts.", blank lines, "---") is not TOC-worthy.
  }
  return entries;
}
...
// This book's six content tags, per STYLE-GUIDE.md §7 - distinct from the sibling
// Sentinel-ops book's six tags, so the CSS class map is rebuilt for this book's own set.
const TAG_COLORS = {
  CONCEPT: "tag-concept",
  HUNTER: "tag-hunter",
  QUERY: "tag-query",
  "FALSE POSITIVE": "tag-false-positive",
  PIVOT: "tag-pivot",
...
function main() {
  ...
  * { box-sizing: border-box; }
  ...
  section.chapter pre { background: #f4f4f4; border: 0.5pt solid var(--baseline); border-left: 3pt solid var(--violet); padding: 6pt; font-size: 7.6pt; overflow-wrap: break-word; word-break: break-word; white-space: pre-wrap; page-break-inside: avoid; }
  section.chapter code { font-family: Consolas, "Courier New", monospace; font-size: 8.1pt; background: var(--violet-tint); padding: 0 2pt; color: var(--ink); overflow-wrap: break-word; word-break: break-word; }
  ...
  .tag-hunter { background: var(--aqua-tint); color: #0d6b49; border: 0.5pt solid var(--aqua); }
  .tag-query { background: var(--blue-tint); color: #164a8a; border: 0.5pt solid var(--blue); }
  .tag-false-positive { background: var(--orange-tint); color: #93401d; border: 0.5pt solid var(--orange); }
  .tag-pivot { background: var(--magenta-tint); color: #99245c; border: 0.5pt solid var(--magenta); }
  .tag-soc-management { background: var(--violet-tint); color: #2e2470; border: 0.5pt solid var(--violet); }
  `;
...
<head>
<meta charset="utf-8">
<title>The SOC Threat Hunting Query Cookbook</title>
<style>${css}</style>
</head>
<body>
<section class="cover">
  <div class="kicker">NESHBOY SOC Professional Library</div>
  <h1>THE SOC THREAT HUNTING QUERY COOKBOOK</h1>
  <div class="accent-rule"></div>
  <h2>Sigma, KQL, SPL, AQL, YARA-L, and EQL Side by Side — Organized by Attacker Behavior, Not by Query Language</h2>
